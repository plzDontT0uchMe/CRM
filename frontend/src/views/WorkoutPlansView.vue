...
const getPrograms = async () => {
    try {
        const resp = await axios.get('/api/getPrograms')
        if (resp.data.successfully) {
            workoutPlans.value = resp.data.programs
        }
    } catch (error) {
        console.error(error)
    }
...
const createPlan = async () => {
    ...
    for (const exercise of plan.exercises) {
        data.append('ids', exercise.id)
    }
    try {
        const resp = await axios.post('/api/createProgram', data)
        if (resp.data.successfully) {
            await getPrograms()
        }
    } catch (error) {
