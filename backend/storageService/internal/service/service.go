...
func AuthorizeAccount(r *authService.AuthorizationRequest) (*models.Account, error, int) {
	...
	if err != nil {
		logger.CreateLog("error", fmt.Sprintf("error get account by login: %v", err), "userLogin", account.Login)
		return nil, err, httpStatus
	}

	err, httpStatus = postgres.UpdateLastActivityByAccountId(account.Id, time.Now().UTC())
	if err != nil {
		logger.CreateLog("error", fmt.Sprintf("error update last activity by account id: %v", err), "userLogin", account.Login)
		return nil, err, httpStatus
	}
...
func RegisterAccount(r *authService.RegistrationRequest) (*models.Account, error, int) {
	...
	if err != nil {
		logger.CreateLog("error", fmt.Sprintf("error create account: %v", err), "userLogin", account.Login)
		return nil, err, httpStatus
	}
	return &account, nil, httpStatus
}
...
func DeleteAllSessionsByAccount(account *models.Account) (error, int) {
	...
	if err != nil {
		logger.CreateLog("error", fmt.Sprintf("error delete all sessions by account: %v", err), "userLogin", account.Login)
...
func CreateSession(account *models.Account) (*models.Session, error, int) {
	...
	if err != nil {
		logger.CreateLog("error", fmt.Sprintf("error create session: %v", err), "userLogin", account.Login)
		return nil, err, httpStatus
	}
...
func CheckAuthorization(r *authService.CheckAuthorizationRequest) (*models.Account, error, int) {
	...
	if err != nil {
		logger.CreateLog("error", fmt.Sprintf("error get session by access token: %v", err))
		return nil, err, httpStatus
	}
	...
	if session.DateExpirationAccessToken.UTC().Compare(time.Now().UTC()) < 0 {
		...
		if err != nil {
			logger.CreateLog("error", fmt.Sprintf("error remove access token by session id: %v", err))
			return nil, err, httpStatus
		}
	}
	...
	if err != nil {
		logger.CreateLog("error", fmt.Sprintf("error update last activity by account id: %v", err))
		return nil, err, httpStatus
	}
	...
	if err != nil {
		logger.CreateLog("error", fmt.Sprintf("error get account by id: %v", err))
		return nil, err, httpStatus
	}
